...
def swap_lesson(list_of_lessons: list, first_lesson: str, second_lesson: str) -> list:
    if first_lesson in list_of_lessons and second_lesson in list_of_lessons:
        ...
        if first_position + 1 in range(len(list_of_lessons)):
            first_lesson_has_exercise = f"{first_lesson}-Exercise" in list_of_lessons[first_position + 1]
        if second_position + 1 in range(len(list_of_lessons)):
            second_lesson_has_exercise = f"{second_lesson}-Exercise" in list_of_lessons[second_position + 1]
        # Swap first to second lessons
        ...
        # Swap exercise
        # Case where both lessons had exercise
...
lessons = input().split(", ")
command = input().split(":")
while len(command) > 1:  # command[0] != "course start"
    action = command[0]
    ...
    command = input().split(":")
for index, lesson_or_exercise_name in enumerate(lessons, start=1):
    print(f"{index}.{lesson_or_exercise_name}")
